...
public class GulimallElasticSearchConfig {
    ...
    public RestHighLevelClient esRestClient() {
        ...
        // Returns a new RestClientBuilder to help with RestClient creation. Creates a
        // new builder instance and sets the nodes that the client will send requests
        // to.
        // 返回一个新的 RestClientBuilder 以帮助创建 RestClient。创建新的生成器实例并设置客户端将向其发送请求的节点。
        // You can use this if you do not have metadata up front about the nodes. If you
        // do, prefer builder(Node...).
        // 如果预先没有有关节点的元数据，则可以使用它。如果这样做，请选择 builder（Node...）。
        final RestClientBuilder builder = RestClient.builder(httpHost);

        // Creates a RestHighLevelClient given the high level RestClientBuilder that
        // allows to build the RestClient to be used to perform requests.
        // 在给定高级 RestClientBuilder 的情况下创建一个 RestHighLevelClient，该 RestClientBuilder
        // 允许生成用于执行请求的 RestClient。
