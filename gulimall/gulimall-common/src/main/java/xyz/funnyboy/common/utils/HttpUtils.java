...
{
    public static HttpResponse doPost(String host, String path, Map<String, String> headers) throws IOException, URISyntaxException {
    ...
    public static HttpResponse doGet(String host, String path, Map<String, String> headers) throws IOException, URISyntaxException {
        List<NameValuePair> nvps = new ArrayList<>();
        for (String key : headers.keySet()) {
            nvps.add(new BasicNameValuePair(key, headers.get(key)));//参数
        }
        //可以加很多参数
        URIBuilder uriBuilder = new URIBuilder(host + path);
        uriBuilder.setParameters(nvps);
        ...
        return HttpClientBuilder
                .create()
                .build()
                .execute(httpGet);//CloseableHttpClient执行方法
    }
